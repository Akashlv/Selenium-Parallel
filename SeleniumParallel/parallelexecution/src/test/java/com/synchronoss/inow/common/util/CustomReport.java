...
public class CustomReport implements IReporter {
	...
	protected void writeBody() {
		...
		writeUserSummary();
		writer.print("<table><h1>Scenario Summary</h1></table>");
		writeScenarioSummary();
		writer.print("<table><h1>Scenario Details</h1></table>");
	...
	protected void writeUserSummary() {
		...
		for (String user : userToTestCaseCntMap.keySet()) {
			...
			hm.put(user, userToTestCaseCntMap.get(user).get("success") + userToTestCaseCntMap.get(user).get("failure")
					+ userToTestCaseCntMap.get(user).get("skipped"));
		...
		writer.print("<tr>");
		writer.print("<td>TCs/DAY</td>");
		LocalDate start = LocalDate.of(2018, 9, 10); // 10th Sep 2018
		LocalDate end = LocalDate.now();

		long daysCnt = calcWeekDays(start, end);
		System.out.println("daysCnt=" + daysCnt);
		if (daysCnt == 0) {
			daysCnt = 1;
		}
		for (String user : userToTestCaseCntMap.keySet()) {
			writer.print("<td>" + hm.get(user) / daysCnt + "</td>");
		}
		writer.print("</tr>");
		writer.print("</tbody>");
		writer.print("</table>");
	...
	protected void updateUserEntry(ITestResult aResult, TreeMap<String, TreeMap<String, Integer>> userToTestCaseCntMap,
			String type) {
		String user = "unknown";
		if (aResult.getMethod().getConstructorOrMethod() != null
				&& aResult.getMethod().getConstructorOrMethod().getMethod() != null
				&& aResult.getMethod().getConstructorOrMethod().getMethod().getAnnotation(TestCase.class) != null
				&& aResult.getMethod().getConstructorOrMethod().getMethod().getAnnotation(TestCase.class)
						.Author() != null) {
			user = Utils.escapeHtml(
					aResult.getMethod().getConstructorOrMethod().getMethod().getAnnotation(TestCase.class).Author())
					.trim();
		}
